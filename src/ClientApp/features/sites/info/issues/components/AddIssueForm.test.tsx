...
const mockMutation = { isPending: false, mutateAsync: jest.fn() };

jest.mock('@/hooks/useColorPalette', () => ({ useColorPalette: () => ({ background: 'white', primary: 'blue', secondary: 'gray', text: 'black' }) }));
jest.mock('../hooks/useCreateIssue', () => ({ useCreateIssue: () => mockMutation }));

describe('AddIssueForm', () => {
  beforeEach(() => {
    mockMutation.mutateAsync.mockReset();
    mockMutation.mutateAsync.mockResolvedValue({ id: 'issue-1' });
  });
  ...
  it('submits trimmed valid values and closes after creation', async () => {
    ...
    await waitFor(() => expect(mockMutation.mutateAsync).toHaveBeenCalledWith({ siteId: 'site-1', title: 'Broken gate', description: 'The gate will not close.' }));
    await waitFor(() => expect(onClose).toHaveBeenCalledTimes(1));
  });

  it('shows local validation errors and surfaces failed submissions', async () => {
    mockMutation.mutateAsync.mockRejectedValue(new Error('Network unavailable'));
